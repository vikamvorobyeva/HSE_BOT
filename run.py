...
if __name__ == "__main__":
    ...
    choose = st.selectbox("Graphics: ", ['Proportion of Fresh, Canned, etc... Fruits and Vegetables', 'Connection between different Forms and Retail price', ' Comparison with one edible cup of food','Comparison Cup equivalent price with its size' ' interrelation (Difference to Price In %) and (Difference to Cup In %)  without Form Juice',' Graphic Fresh',' Graphic Canned',  ' Graphic Juice',  ' Graphic Dried',' Graphic Frozen', ])
    ...
    if choose == 'Connection between different Forms and Retail price':
        response = requests.get()
        img =
    if choose == ' Comparison with one edible cup of food':
        response = requests.get()
        img =
    if choose == 'Comparison Cup equivalent price with its size':
        response = requests.get()
        img =
